...
async function getRecentPosts(): Promise<PostSummary[]> {
  ...
  try {
    const res = await fetch(`${baseUrl}/api/posts?limit=4`, {
      next: { revalidate: 60 },
    });
...
function Article({ post }: { post: PostSummary }) {
  return (
    <Card as="article">
  ...
  return (
    <li className="flex gap-4">
      <div className="relative mt-1 flex h-10 w-10 flex-none items-center justify-center rounded-full shadow-md ring-1 shadow-zinc-800/5 ring-zinc-900/5 dark:border dark:border-zinc-700/50 dark:bg-zinc-800 dark:ring-0">
        <Image src={role.logo} alt="" width={28} height={28} className="h-7 w-7 rounded-full" unoptimized />
      </div>
